...
const PostWidget = ({ postId, postUserId, name, description, location, picturePath, userPicturePath, likes, comments }) => {
  ...
  const isLiked = Boolean(likes[loggedInUserId])   //whether the current user has liked it or not
  ...
  const patchLike = async () => {
    const reponse = await fetch(`http://localhost:8080/posts/${postId}/like`, {
      ...
      body: JSON.stringify({ userId: loggedInUserId }),  //to check if the current user has liked the post or not
    });
